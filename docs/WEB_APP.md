...
「タスクを詳しく登録」は保存前の下書き。現在のブラウザ版の全入力項目と表示開始をまとめて登録する。新規登録はIndexedDBの1トランザクションで親・チェック項目を保存し、キャンセルした下書きは書き込まない。予定・関連ファイル/URLの編集・独立同期は従来どおりWindows版に限定。

PWAは同じURL・同じIndexedDBを使用する。ブラウザ/OSによってホーム画面版が別の保存領域になる場合はGoogle連携で引き継ぐ。beforeinstallpromptは設定画面を開く前から受信し、利用可能な場合にだけ実際のインストールボタンを出す。非対応環境はSafariの共有/Chromeのメニュー手順を案内する。OSのインストールを強制しない。

アクセストークンを永続保存する方針変更はない。ホーム画面から再起動した場合も必要に応じて再接続する。バックグラウンド同期は追加していない。service workerは全公開ファイルの内容からキャッシュ世代を計算し、強制更新・強制再読込をしない。

検証: 390px Edgeモバイルエミュレーション、初回詳細入力、未保存破棄、再読込、オフライン登録、チェック項目保存、PWA用PNGサイズ、インストールイベントのUIを確認。実iPhone/Androidへのインストールは手元に実機がないため未確認。
