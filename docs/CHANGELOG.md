...
- 起動時にメイン・ミニ・登録ウィンドウへブランドアイコンを明示設定。トレイにも同じロゴを使用。
- インストーラーに専用ICOを同梱し、このアプリを指す既存ショートカットのアイコン参照を更新。アプリ識別子を維持し、他のアプリやWindowsのアイコンキャッシュファイルは変更しない。
- 詳細上部に締切の要約と編集欄への移動ボタンを追加。未保存の締切変更も明示。
- 一覧に作業内容の1行プレビューを追加し、締切バッジ・分類色・選択状態を整理。
- チェック項目に進捗バーを追加。設定見出しとキーボード操作時のフォーカス表示を改善。
- DB形式・同期ルール・ブラウザ版の保存形式は変更なし。
